...
from Renderer.render import define_R
...
STROKE_TYPE = 'MyPaintWaterInk'
Decoder = define_R(ACTION_SIZE[STROKE_TYPE],\
	ACTION_SHAPESIZE[STROKE_TYPE], 'dual-render')
Decoder.load_state_dict(torch.load('../last_ckpt.pt')['model_R_state_dict'])
Decoder.eval()
def decode(x, canvas):
...
class DDPG(object):
    def __init__(self, batch_size=64, env_batch=1, max_step=40, \
        ...
        self.actor = ResNet(9, 18, ACTION_SIZE[STROKE_TYPE]*5) # target, canvas, stepnum, coordconv 3 + 3 + 1 + 2
        self.actor_target = ResNet(9, 18, ACTION_SIZE[STROKE_TYPE]*5)
