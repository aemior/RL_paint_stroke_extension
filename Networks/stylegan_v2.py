...
class StyleBlock(nn.Module):
    def __init__(self, input_channel, output_channel, w_dim):
        ...
        self.RGB = nn.Sequential(
            nn.Conv2d(output_channel, 3, 1, 1, 0),
            #nn.Conv2d(8, 3, 1, 1, 0),
        ) 
    ...
    def to_res(self, x):
        return self.RGB(x), self.ALPHA(x)
...
class StyleRender(nn.Module):
    def __init__(self, action_size, noise_dim=4, dim=32, w_dim=512):
        ...
        self.main = nn.ModuleDict({
            "L1":StyleBlock(dim*16, dim*32, w_dim=w_dim), #8*8 /32 init
            "L2":StyleBlock(dim*32, dim*32, w_dim=w_dim), #16*16
            "L3":StyleBlock(dim*32, dim*4, w_dim=w_dim), #32*32
